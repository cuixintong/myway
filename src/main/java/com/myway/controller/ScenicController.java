...
import com.myway.utils.UploadUtil;
...
public class ScenicController {
    ...
    public String insert(Scenic scenic,MultipartFile scenicImage){
        ...
        if (scenicImage.getSize() != 0) {
            //将上传的文件保存到磁盘中
            String path =
                    "E:\\compilerspace\\IDEA\\idea\\下载的case\\myway-master\\myway-master\\src\\main\\webapp\\image\\scenic\\pic";
    ...
    public String update(Scenic scenic,MultipartFile scenicImage){
        ...
        if (scenicImage.getSize() != 0) {
            //将上传的文件保存到磁盘中
            String path = "E:\\workspace_IDEA\\myway\\src\\main\\webapp\\image\\scenic\\pic";
            String imageName = UploadUtil.upload(scenicImage,path);
            //将图片路径封装到Scenic中
            scenic.setPic("image/scenic/pic/"+imageName);
        }
